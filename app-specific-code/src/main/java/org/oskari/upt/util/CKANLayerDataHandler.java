...
import org.oskari.upt.helpers.FileHelper;
import org.oskari.upt.helpers.LayerHelper;
import org.oskari.upt.helpers.LayerJSONHelper;
import org.oskari.upt.parser.CKANDataParser;
import org.oskari.upt.parser.CKANOrganization;
...
public class CKANLayerDataHandler {
    ...
    /**
     * Parses the given CKAN resource JSON, gets capabilities from the supported
     * APIs
     ...
     */
    public static void addLayersFromCKANJSONResource(JSONObject resource, boolean isPrivateResource,
            ...
            switch ((format).toLowerCase()) {
                case "wms":
                    addWMSLayers(resource, connection, capabilitiesService, url, user, pw, currentCrs,
                            isPrivateResource, organization, null, false);
                    break;
                case "wmts":
                    addWMTSLayers(resource, connection, capabilitiesService, url, user, pw, currentCrs,
                            isPrivateResource, organization, false);
                    break;
                case "wfs":
                ...
                case "esri rest":
                    ...
                    break;
                case "shp":
                    addShpFileAsLayer(resource, connection, capabilitiesService, url, user, pw, currentCrs,
                            isPrivateResource, organization);
                    break;
                case "tif":
                case "tiff":
                    addGeoTIFFAsLayer(resource, connection, capabilitiesService, url, user, pw, currentCrs,
                            isPrivateResource, organization);
                    break;
                default:
     ...
     * 
     * @param resource a CKAN resource as JSON object (each object of the resources
     *                 array in /api/action/package_show CKAN API call).
    ...
    private static void updateUrlToCKAN(JSONObject resource, String wfsUrl, String wmsUrl, String apiKey) {
        try {
            ...
            // curl -X POST http://ckan.url/api/action/resource_patch -H "Authorization:
            // the-sysadmin-api-key" -d '{"id": "the-resource-file-uuid", "wms_url":
            // "http://the.wms.url", "wfs_url": "http://the.wfs.url"}'
    ...
    private static boolean checkResourceStatus(JSONObject resource, Connection connection) {
        ...
        String uuid = (String) resource.get("id");
        String lastModified = (resource.get("last_modified") != null) ? (String) resource.get("last_modified")
    ...
    private static boolean resourceNeedsUpdate(Connection connection, String uuid, String lastModified) {
        ...
        if (currentModifiedDate != null
                && (currentModifiedDate.isEqual(lastModifiedDate) || currentModifiedDate.isAfter(lastModifiedDate))) {
            return false;
        }
    ...
    private static void addWMSLayers(JSONObject resource, Connection connection,
            ...
            String currentCrs, boolean isPrivateResource, CKANOrganization organization, String overrideName,
            boolean addForceProxy) throws ServiceException {
        ...
        addLayers(connection, url, user, pw, currentCrs, json, OskariLayer.TYPE_WMS, isPrivateResource, mainGroupName,
                organization, overrideName, addForceProxy);
        addOrUpdateResourceInfo(resource, connection);
    }

    private static void addWMTSLayers(JSONObject resource, Connection connection,
            CapabilitiesCacheService capabilitiesService, String url, String user, String pw,
            String currentCrs, boolean isPrivateResource, CKANOrganization organization, boolean addForceProxy)
            throws ServiceException {
        ...
        OskariLayerCapabilities caps = capabilitiesService.getCapabilities(url, OskariLayer.TYPE_WMTS, version, user,
                pw);
        ...
        try {
            ...
            addLayers(connection, url, user, pw, currentCrs, resultJSON, OskariLayer.TYPE_WMTS, isPrivateResource,
                    mainGroupName, organization, null, addForceProxy);
            addOrUpdateResourceInfo(resource, connection);
    ...
    private static void addLayers(Connection connection, String url, String user, String pw, String currentCrs,
        ...
        try {
            ...
            boolean useDefaultLocales = PropertyUtil.getOptional("ckan.integration.ckanapi.layer.defaultlocales",
                    false);
            ...
            org.json.JSONObject layerPermissions = LayerJSONHelper
                    .getRolePermissionsJSONForRoleAndAdmin(organization.getName());
            ...
            for (int i = 0; i < layers.length(); i++) {
                ...
                layersToAdd.put(LayerHelper.generateLayerJSON(layerType, url, layerName, mainGroupName,
                        layerLocale, false, -1,
                        null, -1.0, -1.0, null, null, null, null, null, null, false, 0, currentCrs,
    ...
    private static void addShpFileAsLayer(JSONObject resource, Connection connection,
            CapabilitiesCacheService capabilitiesService,
            String url, String user, String pw, String currentCrs, boolean isPrivateResource,
            CKANOrganization organization) throws ServiceException {
        boolean publishWFS = (resource.get("publish_wfs") != null && !((String) resource.get("publish_wfs")).equals(""))
        ...
        boolean createResourceWorkspaces = PropertyUtil.getOptional("ckan.integration.ckanapi.shp.resourceworkspaces",
                true);
        ...
        try {
            ...
            uploadSldToGeoServer(gsUrl, new File(dataFilePath), workspaceName,
    ...
    private static void uploadShpFileToGeoServer(String workspaceName, String storeName,
        ...
        HttpPut httpPut = new HttpPut(
                String.format("%s/rest/workspaces/%s/datastores/%s/file.shp", gsUrl, workspaceName, storeName));
    ...
    private static void uploadSldToGeoServer(String gsUrl, File styleFile, String workspaceName, String shpName)
            throws IOException {
        ...
        // NOTE! For some unknown reason, GeoServer seems to require double encoding
        // spaces
        // (and probably other special characters) in layer names. That is why the
        // shpName
        ...
        if (styleName != null && shpName != null) {
            ...
            httpPut.setEntity(new StringEntity(
                    String.format("<layer><defaultStyle><name>%s</name></defaultStyle></layer>", styleName),
    ...
    private static void addGeoTIFFAsLayer(JSONObject resource, Connection connection,
            CapabilitiesCacheService capabilitiesService,
            String url, String user, String pw, String currentCrs, boolean isPrivateResource,
            CKANOrganization organization) throws ServiceException {
        boolean createResourceWorkspaces = PropertyUtil
                .getOptional("ckan.integration.ckanapi.geotiff.resourceworkspaces", false);
    ...
    private static void uploadGeoTIFFToGeoServer(String workspaceName, String storeName,
            ResponseHandler<String> responseHandler,
            String gsUrl, File tiffFile) throws IOException {
        CloseableHttpClient httpclient = HttpClients.createDefault();
        String authHeaderValue = LayerHelper.generateGeoServerAuthHeader();
        HttpPut httpPut = new HttpPut(
                String.format("%s/rest/workspaces/%s/coveragestores/%s/file.geotiff", gsUrl, workspaceName, storeName));
    ...
    private static void createGeoServerWorkspace(String workspaceName, ResponseHandler<String> responseHandler,
        ...
        httpPost.setEntity(new StringEntity(String.format("<workspace><name>%s</name></workspace>", workspaceName),
                ContentType.create("text/xml")));
        httpPost.setHeader("Authorization", authHeaderValue);
    ...
    private static void deleteGeoServerStore(String storeType, String workspaceName, String storeName,
            ResponseHandler<String> responseHandler,
            String gsUrl) throws IOException {
        CloseableHttpClient httpclient = HttpClients.createDefault();
        String authHeaderValue = LayerHelper.generateGeoServerAuthHeader();
        HttpDelete httpDelete = new HttpDelete(
                String.format("%s/rest/workspaces/%s/%ss/%s?recurse=true", gsUrl, workspaceName, storeType, storeName));
        httpDelete.setHeader("Authorization", authHeaderValue);
